...
class IatRecorder {
  constructor (config) {
    ...
    this.appId = '5f5b0f76' 
    this.apiKey = '7dd1c9cba9f6934e17ac3592f9072826'
  }
  ...
  constructor () {
    var iatRecorder = new IatRecorder({
      ...
      onMessage: (message) =>{
        ...
        let res = JSON.parse(message).cn.st.rt;
        let resArray = [];
        res[0].ws.map((item)=>{
          resArray.push(item.cw[0].w)
        });
        console.log('resArray____',resArray);
      },
      onStart: () => {
        $('hr').addClass('hr')
